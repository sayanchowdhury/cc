...
struct GreaterGameDiv2 {
    int
    calc(vector<int> snuke, vector<int> sothe) {
        int c=0;
        int i;
...
bool do_test(vector<int> snuke, vector<int> sothe, int __expected, int caseNo) {
    ...
    if (__result == __expected) {
        ...
        cout << " GreaterGameDiv2." << COLOR_GREEN << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
        return true;
    }
    else {
        cout << COLOR_RED << "[  FAILED  ]" << COLOR_CLEAR;
        cout << " GreaterGameDiv2." << COLOR_RED << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
...
int run_testcase(int __no) {
    switch (__no) {
        case 0: {
            ...
            int __expected = 1;
            #ifdef LOCALHOST
            return do_test(to_vector(snuke), to_vector(sothe), __expected, __no);
            #endif
            int ret = do_test(to_vector(snuke), to_vector(sothe), __expected, __no);
            if (! ret) {
                debug (to_vector(snuke), to_vector(sothe));
                cout << endl;
            }
            return ret;
        }
