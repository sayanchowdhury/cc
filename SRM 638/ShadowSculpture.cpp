...
struct ShadowSculpture {
    string
    possible(vector<string> XY, vector<string> YZ, vector<string> ZX) {
        return "";
    }
};
...
bool do_test(vector<string> XY, vector<string> YZ, vector<string> ZX, string __expected, int caseNo) {
    ...
    if (__result == __expected) {
        ...
        cout << " ShadowSculpture." << COLOR_GREEN << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
        return true;
    }
    else {
        cout << COLOR_RED << "[  FAILED  ]" << COLOR_CLEAR;
        cout << " ShadowSculpture." << COLOR_RED << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
...
int run_testcase(int __no) {
    switch (__no) {
        case 0: {
            ...
            #ifdef LOCALHOST
            return do_test(to_vector(XY), to_vector(YZ), to_vector(ZX), __expected, __no);
            #endif
            int ret = do_test(to_vector(XY), to_vector(YZ), to_vector(ZX), __expected, __no);
            if (! ret) {
                debug (to_vector(XY), to_vector(YZ), to_vector(ZX));
                cout << endl;
            }
            return ret;
        }
