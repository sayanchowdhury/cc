...
struct NamingConvention {
    string
    toCamelCase(string variableName) {
        return "";
    }
};
...
bool do_test(string variableName, string __expected, int caseNo) {
    ...
    if (__result == __expected) {
        ...
        cout << " NamingConvention." << COLOR_GREEN << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
        return true;
    }
    else {
        cout << COLOR_RED << "[  FAILED  ]" << COLOR_CLEAR;
        cout << " NamingConvention." << COLOR_RED << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
...
int run_testcase(int __no) {
    switch (__no) {
        case 0: {
            ...
            #endif
            int ret = do_test(variableName, __expected, __no);
            if (! ret) {
                debug (variableName);
                cout << endl;
            }
