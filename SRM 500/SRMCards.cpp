...
bool do_test(vector<int> cards, int __expected, int caseNo) {
    ...
    if (__result == __expected) {
        ...
        cout << " SRMCards." << COLOR_GREEN << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
        return true;
    }
    else {
        cout << COLOR_RED << "[  FAILED  ]" << COLOR_CLEAR;
        cout << " SRMCards." << COLOR_RED << "Test" << caseNo << " " << COLOR_CLEAR;
        cout << "(" << elapsed << " seconds)" << endl;
...
int run_testcase(int __no) {
    switch (__no) {
        case 0: {
            ...
            #endif
            int ret = do_test(to_vector(cards), __expected, __no);
            if (! ret) {
                debug (to_vector(cards));
                cout << endl;
            }
            return ret;
        }
